...
public class ContentDao implements IDao{
	...
	@Override
	public void writeDao(final Long mId, final String mWriter, final String mContent) {
		...
		this.template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
					throws SQLException {
				String query = "insert into board (mId, mWriter, mContent) values (?, ?, ?)";
				PreparedStatement pstmt = con.prepareStatement(query);
				pstmt.setLong(1, mId);
				pstmt.setString(2, mWriter);
